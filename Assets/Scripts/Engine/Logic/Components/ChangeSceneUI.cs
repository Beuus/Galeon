...
public class ChangeSceneUI : MonoBehaviour {
    ...
    {
        //SceneMgr.OnAsyncLoadingProgress
        //SceneMng.LoadingAsync(sceneToGo, OnAsyncLoadingProgress onAsyncLoadingProgress);
        //TODO 1 cambiamos de escena o hacemos desapilamos la escena en función de los que indique el inspector.
        
        SceneMgr sceneMng = GameMgr.GetInstance().GetServer<SceneMgr>();

        if (m_return) { 
            //Debug.LogError("Desapilando la escena en la cima de la pila");
            sceneMng.PopScene(true);
        }
        else {
            //Debug.LogError("Cambiando de escena a "+ sceneToGo);
            //sceneMng.PushScene(sceneToGo);
            sceneMng.ChangeScene(sceneToGo, m_nextSceneAfterTransitionScene);
        }
